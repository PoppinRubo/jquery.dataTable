...
class DB
{
    ...
    protected $_pass = '';
    protected $_dbName = 'jquery.datatable'; //数据库名
    protected $_sql = false; //最后一条sql语句
    protected $_where = '';
    protected $_order = '';
    protected $_limit = '';
    protected $_field = '*';
    protected $_count = false;//标识是否是获取条数,false代表是获取结果集
    protected $_find = false;//标识是否是查询一条，返回为一条数组结果，而非结果集
    ...
    protected function _doQuery($sql='') {
        ...
        if ($this->_count){
            $this->_count=false;//关闭状态
            return count($result);
        }
        if ($this->_find){
            $this->_find=false;//关闭状态
            return $result[0];
        }
        return $result;
    }
    ...
    public function count($tbName=''){
        ...
        $this->_clear = 1;
        $this->_clear();
        $this->_count=true;//现在是要获取条数而不是结果集
        return $this->_doQuery(trim($sql));
    }
    /**
     *查找一条
    ...
    public function find($tbName=''){
        ...
        $this->_clear = 1;
        $this->_clear();
        $this->_find=true;//现在是要获取条数而不是结果集
        return $this->_doQuery(trim($sql));
    }
    ...
    public static function ShowError($errMsg){
        echo '<div style="width:100%;background-color:#ABCDEF;color:#232323;font-size:24px;text-align:center;padding:20px 0;">
				' .$errMsg.'
		</div>';
    }
    /**
